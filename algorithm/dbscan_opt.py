    ...
    def fit(self):
        ...
        for point_idx in core_points_idx:
            ...
            if (cluster_labels[point_idx] == 0):
                ...
                while len(seeds) > 0 or len(near_seeds) > 0:
                    if len(seeds) > 0:
                        # 弹出一个新种子点
                        new_point = seeds.pop()
                        # 将new_point标记为当前类
                        ...
                        query_result = np.where(self.score_matrix[:,new_point]<=self.eps_soft)[0]
                        ...
                        if len(query_result) >= self.min_pts:
                            ...
                            for resultPoint in query_result:
                                if cluster_labels[resultPoint] == 0:
                                    near_seeds.add(resultPoint)
                    if len(near_seeds) > 0:
                        # 弹出一个新种子点
                        new_point = near_seeds.pop()
                        # 将new_point标记为当前类
                        cluster_labels[new_point] = cur_label
                        # 寻找new_point种子点self.eps邻域（包含自己）
